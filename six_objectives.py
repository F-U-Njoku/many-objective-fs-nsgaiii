...
class featSel(BinaryProblem):
    ...
    def __init__(self, x: pd.DataFrame, y: pd.Series, pop_size: int, classifier, sf):
        ...
        self.obj_labels = ["subset size", "accuracy", "f1", "vif", "dp", "eo"]
        self.count = 0
    ...
    def evaluate(self, solution: BinarySolution) -> BinarySolution:
        ...
        for train_index, test_index in skf.split(self.F[idx], self.target):
            clf_temp = self.classifier.fit(self.F[idx].iloc[train_index], self.target.iloc[train_index])
            y_pred = clf_temp.predict(self.F[idx].iloc[test_index])
            dp.append(demographic_parity_ratio(self.target.iloc[test_index], y_pred,
                                               sensitive_features=self.F[self.sf].iloc[test_index]))
            eo.append(equalized_odds_ratio(self.target.iloc[test_index], y_pred,
                                           sensitive_features=self.F[self.sf].iloc[test_index]))
        dp = np.mean(dp)
        eo = np.mean(eo)
        ...
        solution.objectives[4] = dp * -1.0
        solution.objectives[5] = eo * -1.0
...
def experiment():
    ...
    problem = featSel(x, y_true, population_size, classifier, sf)

    algorithm = NSGAIII(
        problem=problem,
        population_size=population_size,
        reference_directions=UniformReferenceDirectionFactory(6, n_points=population_size - 1),
    ...
    f.write("Subset size, Accuracy, F1-Score, VIF, Statistical parity, Equalised odds, Solution")
